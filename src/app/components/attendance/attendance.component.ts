...
export class AttendanceComponent implements OnInit {
  ...
  loadAttendanceData(): void {
    ...
    if (this.isStudent()) {
      ...
      this.attendanceService.getStudentAttendance(currentUser.id).subscribe(response => {
        if (response.success) {
      ...
      // Load student stats
      this.attendanceService.getStudentAttendanceStats(currentUser.id).subscribe(response => {
        if (response.success) {
  ...
  filterByDate(): void {
    if (this.selectedDate) {
      this.attendanceService.getAttendanceByDate(this.selectedDate).subscribe(response => {
        if (response.success) {
          this.teacherAttendanceRecords = response.data;
        }
      });
